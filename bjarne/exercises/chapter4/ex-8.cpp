// 8. There is an old story that the emperor wanted to thank the inventor of the
// game of chess and asked the inventor to name his reward.
// The inventor asked
...
// need a loop, of course, and probably an
// int to keep track of which square you are at,
// an int to keep the number of grains on the current square,
// and an int to keep track of the grains on all previous squares.
// We suggest that you write out the value of all your variables for each
// iteration of the loop so that you can see what’s going on.

#include <iostream>

int main() {
  int currentRiceCount = 1;
